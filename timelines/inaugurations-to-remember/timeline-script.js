...
var timelineTitle = 'Presidential Inaugurations To Remember';
...
$(function() {
    ...
    function getTimestamp(year, beginning) {
        if (beginning) return Date.parse('January 1, ' + year);
        else return Date.parse('December 31, ' + year) + 86400000 - 1; // plus 1 day, minus 1 millisecond
    }
    /**
    ...
    function setupTimeline(data, tabletop) {
        ...
        // write the title to the title div
        ...
        $.each(tabletop.sheets(timelineConfig.sheetName).all(), function(i, val) {
            ...
            if (years.indexOf(year) < 0) years[years.length] = year;
        ...
        $.each(years, function(i, val) {
            var timestamp;
            if (direction == 'newest') timestamp = getTimestamp(val, false);
            else timestamp = getTimestamp(val, true);
            var context = {
                year: val,
                timestamp: timestamp
            };
...
$.Isotope.prototype._spineAlignLayout = function($elems) {
    ...
    $elems.each(function(i, val) {
        ...
        if (i == $elems.length - 1) $this.addClass('last');
        var x, y;
