...
*/
    ...
    if(window.innerWidth < breakPoint) {
      width = (window.innerWidth * .295 - margin.left - margin.right);
      height = 130;      }
    else {
    ...
    yAxis = d3.svg.axis().scale(yScale).orient("left").ticks(3).outerTickSize(0).tickSubdivide(1).tickSize(-width).tickFormat(function(d){return d + "%"});;
...
/*
            lines.append("line")          // attach a line
    .style("stroke", "red")  // colour the line
    .attr("x1", 125)     // x position of the first end of the line
    .attr("y1", 0)      // y position of the first end of the line
    .attr("x2", 125)     // x position of the second end of the line
...
*/
        ...
        lines.append("text").attr("class", "static_year").attr("text-anchor", "start").style("pointer-events", "none").attr("dy", 13).attr("y", height).attr("x", 0).text(function(c) {
          return "20" + formatDate(xValue(c.values[0]));
        });
        lines.append("text").attr("class", "static_year").attr("text-anchor", "end").style("pointer-events", "none").attr("dy", 13).attr("y", height).attr("x", width).text(function(c) {
          return "20" + formatDate(xValue(c.values[7]));
        });
...
/*      d3.selectAll(".static_year").classed("hidden", true);
  ...
  $(function() {
    ...
    queue().defer(d3.csv, "data/selectcounties.csv").await(display);
  });
